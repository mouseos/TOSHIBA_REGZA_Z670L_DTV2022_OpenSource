...
reg_t compare;
    ...
    for( ;; )
    {
        ...
        count = mips_getcount();
        //HS_LOGI("count is 0x%08x,compare is 0x%08x,cyclesPerTick is %d\n",count,compare,cyclesPerTick);
        if( compare < count )
        {
...
{
#ifdef HS_CONFIG_PLATFORM_QEMU
    //ticks is variant on QEMU
    return 1100;
#else
    uint32_t x, y;
