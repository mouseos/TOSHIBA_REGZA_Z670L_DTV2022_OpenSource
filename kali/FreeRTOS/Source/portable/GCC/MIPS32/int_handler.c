...
__attribute__((long_call)) void mips_flush_cache (void);
extern void _gic_interrup_handler(int irq);
...
uint32_t stride = ( mips32_getintctl() & INTCTL_VS );
    ...
    pvISR = ( uint32_t * )( (( mips32_getebase() | 0x20000000) & 0xfffff000 ) + offset );

    /* Copy the jump */
    for (i = 0; i < 4; i ++){
        *((uint32_t*)pvISR + i) = *((uint32_t*)prvInterruptHandoff + i);
    }
...
void vApplicationSetupSoftwareInterrupt( void )
{
    ...
    pvPortInstallISR( HW3, vGicInterruptHandler );
}
