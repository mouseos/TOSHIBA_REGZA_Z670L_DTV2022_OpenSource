...
# endif /* !CONFIG_SPI */
		...
		ulong dev_addr = CONFIG_SYS_DEF_EEPROM_ADDR;
		ulong data = simple_strtoul (argv[2], NULL, 16);
		uchar ucBootFlg;
		int rcode;

		eeprom_init ();

		if (strcmp (argv[1], "rd_bf") == 0) {

			rcode = eeprom_read (dev_addr, 0, &ucBootFlg, 1);
			if (rcode == 0) {
				printf ("BootFlag = 0x%x\n", ucBootFlg);
			}
			else {
				printf ("Read BootFlag Error\n");
			}
			return rcode;

		} else if (strcmp (argv[1], "wr_bf") == 0) {

			ucBootFlg = (uchar)data;
			printf ("Write BootFlag = 0x%x\n", ucBootFlg);
			rcode = eeprom_write (dev_addr, 0, &ucBootFlg, 1);
			if (rcode == 0) {
				printf ("Write BootFlag Success\n");
			}
			else {
				printf ("Read BootFlag Error\n");
			}
