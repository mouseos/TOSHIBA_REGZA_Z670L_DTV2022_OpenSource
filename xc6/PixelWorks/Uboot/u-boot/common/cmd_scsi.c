...
#ifdef CONFIG_XCODE 
u32 g_XCode_Sata_Initialized = 0; 
#endif
...
void scsi_scan(int mode)
{
	...
	for(i=0;i<CONFIG_SYS_SCSI_MAX_SCSI_ID;i++) {
		...
		for(lun=0;lun<CONFIG_SYS_SCSI_MAX_LUN;lun++) {
			pccb->lun=lun;TR
			pccb->pdata=(unsigned char *)&tempbuff; TR
			pccb->datalen=512;TR
			scsi_setup_inquiry(pccb);TR
			if(scsi_exec(pccb)!=TRUE) {
				if(pccb->contr_stat==SCSI_SEL_TIME_OUT) {TR
			...
			if((modi&0x80)==0x80) /* drive is removable */
				scsi_dev_desc[scsi_max_devs].removable=TRUE;TR
			...
			scsi_ident_cpy((unsigned char *)&scsi_dev_desc[scsi_max_devs].revision[0],
				       &tempbuff[32], 4);
			scsi_dev_desc[scsi_max_devs].target=pccb->target;TR
			...
			if(scsi_exec(pccb)!=TRUE) {
				if(scsi_dev_desc[scsi_max_devs].removable==TRUE) {
					scsi_dev_desc[scsi_max_devs].type=perq;TR
					goto removable;
				}
				scsi_print_error(pccb);TR
				continue;
			}TR
			...
			scsi_dev_desc[scsi_max_devs].lba=capacity;TR
			scsi_dev_desc[scsi_max_devs].blksz=blksz;TR
			scsi_dev_desc[scsi_max_devs].type=perq;TR
...
void scsi_init(void)
{
	...
	for(i=0 ; i < CONFIG_SYS_SCSI_MAX_LUN; i++)
	{
		scsi_low_level_init(i);
	}
	scsi_scan(1);
#else
	int busdevfunc;
...
{
	if (!g_XCode_Sata_Initialized)
	{
		scsi_init();
		g_XCode_Sata_Initialized = 1;
	}
