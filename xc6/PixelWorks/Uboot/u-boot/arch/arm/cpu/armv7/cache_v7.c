...
static void v7_dcache_inval_range(u32 start, u32 stop, u32 line_len)
{
	 ...
	 */
	if (start & (line_len - 1)) {
		printf("ERROR: %s - start address is not aligned - 0x%08x - 0x%08x cache line %d\n",
			__func__, start, stop, line_len);
	 ...
	 */
	if (stop & (line_len - 1)) {
		printf("ERROR: %s - stop address is not aligned - 0x%08x - 0x%08x cache line %d\n",
			__func__, start, stop, line_len);
