...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"sata_clktype=0\0" \
	"sata_ssmode=1\0" \
	"set_bootargs=setenv bootargs ethaddr=$ethaddr eth1addr=$eth1addr sata_clkfreq=$sata_clkfreq sata_clktype=$sata_clktype root=/dev/sda2\0" \
	"loadaddr=0x10000000\0" \
	"verifyaddr=0x20000000\0" \
	"flash_kernel_ofs=0x1fc0000\0" \
	"flash_kernel_sz=0x680000\0" \
	"flash_boot_ofs=0x80000\0" \
	"flash_boot_sz=0x80000\0" \
	"eraseenv=dcache off; sf probe; sf erase 3fc0000 +40000; dcache on\0" \
	"bootfile=/boot/uImage_arm_gz\0" \
	"ipaddr=172.23.10.55\0" \
	"serverip=172.23.10.4\0" \
	"Board_ID=0x0001\0" \
	...
	"prog_kernel=dcache off;sf probe;sf erase $flash_kernel_ofs +$flash_kernel_sz;sf write $loadaddr $flash_kernel_ofs $flash_kernel_sz;sf read $verifyaddr $flash_kernel_ofs $flash_kernel_sz;dcache on;cmp.b $loadaddr $verifyaddr $flash_kernel_sz\0" \
	"bootf=dcache off;sf probe;sf read $loadaddr $flash_kernel_ofs $flash_kernel_sz;dcache on;bootm\0" \
	"boots=scsi scan;ext2load scsi 0 10000000 /boot/uImage_arm_gz;bootm\0" \
	"ethaddr=00:88:23:84:17:41\0" \
	"eth1addr=00:88:43:84:47:81\0" \
	"current_active=nand0,4\0" \
	"watchdog=off\0" \
...
#define CONFIG_XCODE6_BOARDID	(0x0001)
