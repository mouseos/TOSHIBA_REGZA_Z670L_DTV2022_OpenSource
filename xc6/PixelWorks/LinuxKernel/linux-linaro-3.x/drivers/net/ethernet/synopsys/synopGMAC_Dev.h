...
//#define TR_INTR_ENABLE        // enable transmit interrupt,
#define IPC_OFFLOAD             // enable HW IP/TCP checksum
#define SYNOPSYS_TSO_ENABLE     // enable TSO
#define ENH_DESC		// enable jumbo support

#define SYNOPSYS_GETH_ENABLE
...
#ifdef CONFIG_ARM
#define NON_CACHE_ADDR(addr) addr
#elif defined(CONFIG_MIPS_XCODE)
#define NON_CACHE_ADDR(addr) KSEG1ADDR(addr)
#else
#define NON_CACHE_ADDR(addr) ((unsigned int)addr - 0x80000000 + 0xC0000000) 
#endif

// Foe Viper stack FPGA, mclk is slow, to workaround for recv crc error
// disable this on standlone FPGA and real ASIC

//#define VIPER_STACK_FPGA

#define TRIM_SKB_LENGTH(x)  ((x) & 0xFFFFFFE0)  // length align to cache line
...
#define TRANSMIT_DESC_SIZE  2048 //768 //384  //12//256	//Tx Descriptors needed in the Descriptor pool/queue
#define RECEIVE_DESC_SIZE   512   //12//256	//Rx Descriptors needed in the Descriptor pool/queue
#define TSO_BUF_SIZE		(TRANSMIT_DESC_SIZE * 1514)	// uncache buffer to optimize
...
#define EHEA_LRO_MAX_AGGR       64 /*32*/ /*16*/ /*8*/ 
...
#define DEFAULT_MAC_ADDRESS {0x00, 0x55, 0x7B, 0xB5, 0x88, 0x89}
...
typedef struct synopGMACDeviceStruct      
{
  ...
  u32 eth_num;
  spinlock_t lock; /* reg read and write lock */
  spinlock_t tx_desc_lock; /* transmit descriptor lock */	
  u32 baseAddr;
  struct net_device * netdev;
  u32 reg;

  u32 GMAC_Power_down; // This global variable is used to indicate the ISR whether the interrupts occured in the process of powering down the mac or not
  ...
  /* for TSO implentation optimization */
  int  tso_index;
  char *tso_buf, *tso_buf_p;  
...
enum MiiRegisters
{
  ...
  PHY_LNK_PART_ABI_REG      = 0x0005,		/*Link Partner Ability Register (Base Page)*/
...
enum GmacRegisters              
{
  ...
  GmacLPICtrlStatus  = 0x0030,	  /* LPI control and status register		*/
...
enum DmaStatusReg         
{ 
  GmacLPIIntr			  = 0x40000000, /* Low Power Logic interrupt */
  GmacTTIIntr			  = 0x20000000,	/* Timestamp Trigger Interrupt */
...
{ 
  ...
  DmaRxStoreAndForward    = 0x02000000,   /* (RSF)Rx Store and forward                        25      RW        0       */                  
...
**********Default Descritpor structure to support 8K buffer per buffer ****************************
DmaRxBaseAddr     = 0x000C,   CSR3 - Receive Descriptor list base address       
DmaRxBaseAddr is the pointer to the first Rx Descriptors. the Descriptor format in Little endian with a
32 bit Data bus is as shown below 
...
//   DmaBusModeInit         = DmaFixedBurstEnable |   DmaBurstLength8   | DmaDescriptorSkip4       | DmaResetOff,
  ...
  DmaIntRxAbnMask         = DmaIntRxNoBuffer | DmaIntRcvOverflow,         /* receiver abnormal interrupt */
  ...
  DmaIntEnable_NAPI_TX    =  DmaIeAbnormal    | DmaIntErrorMask
                           | DmaIntTxAbnMask | DmaIntTxNormMask | DmaIntTxStoppedMask,

  DmaIntEnable_NAPI        =  DmaIeAbnormal    | DmaIntErrorMask,
...
int xc3_dma_tx_set_tso(struct net_device *netdev, synopGMACdevice * gmacdev, u32 tso_header, u32 tso_hlen, u32 tso_data, u32 tso_dlen, u32 skb);
...
//int xc3_tso_start_xmit(struct net_device *netdev, struct sk_buff *skb, synopGMACdevice *gmacdev);
...
static u32  plat_ReadReg(synopGMACdevice *Dev, u32 addr)
{
	...
	if (Dev->reg ==0)
    {
    	while( ( (readl( (unsigned int long *) (Dev->baseAddr  + ETH_STATUS_REG)) ) & ETH_STATUS_REG_CSR_BUSY_MASK )!=0)
			;
		writel(addr, (unsigned int long  *)(Dev->baseAddr + ETH_CSR_CTRL_REG) ) ;
		
		while( ( (readl( (unsigned int long *) (Dev->baseAddr  + ETH_STATUS_REG)) ) & ETH_STATUS_REG_CSR_BUSY_MASK )!=0)
			;
		data = readl((unsigned int long *)(Dev->baseAddr  + ETH_RDAT_REG));
    ...
    {
    	while( ( (readl( (unsigned int long *) (Dev->baseAddr  + ETH_STATUS_REG1)) ) & ETH_STATUS_REG1_CSR_BUSY_MASK )!=0)
			;
		writel(addr, (unsigned int long  *)(Dev->baseAddr + ETH_CSR_CTRL_REG1) ) ;
		while( ( (readl( (unsigned int long *) (Dev->baseAddr  + ETH_STATUS_REG1)) ) & ETH_STATUS_REG1_CSR_BUSY_MASK )!=0)
			;
		data = readl((unsigned int long *)(Dev->baseAddr  + ETH_RDAT_REG1));
    }
    #endif
...
static void  plat_WriteReg(synopGMACdevice *Dev, u32 addr,u32 data)
{
    unsigned long flags;
	...
	if(Dev->reg == 0)
	{
    	while( ( (readl( (unsigned int long *) (Dev->baseAddr  + ETH_STATUS_REG)) ) & ETH_STATUS_REG_CSR_BUSY_MASK )!=0)
			;
		writel(data, (unsigned int long *)(Dev->baseAddr + ETH_WDAT_REG) ) ;
		writel(addr, (unsigned int long *)(Dev->baseAddr + ETH_CSR_CTRL_REG) );
		//while( ( (readl( (unsigned int long *) (Dev->baseAddr  + ETH_STATUS_REG)) ) & ETH_STATUS_REG_CSR_BUSY_MASK )!=0)
			//;
	}
    ...
    else if (Dev->reg ==1)
    {
    	...
    	while( ( (readl( (unsigned int long *) (Dev->baseAddr  + ETH_STATUS_REG1)) ) & ETH_STATUS_REG1_CSR_BUSY_MASK )!=0)
			;
		writel(data, (unsigned int long *)(Dev->baseAddr + ETH_WDAT_REG1) ) ;
		writel(addr, (unsigned int long *)(Dev->baseAddr + ETH_CSR_CTRL_REG1) );
		//while( ( (readl( (unsigned int long *) (Dev->baseAddr  + ETH_STATUS_REG1)) ) & ETH_STATUS_REG1_CSR_BUSY1_MASK )!=0)
			//;
	}
    #endif
	spin_unlock_irqrestore (&Dev->lock, flags);
...
static bool __inline__ plat_CheckBits(synopGMACdevice *Dev, u32 addr, u32 BitPos)
{
	u32 data = plat_ReadReg(Dev, addr);
	data &= BitPos;
  	if(data)  
		return true;
  	else
...
#define synopGMACWriteReg(RegBase, RegOffset, RegData)  plat_WriteReg(gmacdev,(((u32)(RegBase))+RegOffset),RegData)
#define synopGMACReadReg(RegBase,  RegOffset)  plat_ReadReg(gmacdev,(((u32)(RegBase))+RegOffset))
#define synopGMACCheckBits(RegBase,RegOffset,  BitPos)  plat_CheckBits(gmacdev,(((u32)(RegBase))+RegOffset),BitPos)


#define synopGMACSetBits(RegBase, RegOffset, BitPos) \
{ \
	u32 data = synopGMACReadReg(RegBase,  RegOffset); \
    data |= BitPos; \
	synopGMACWriteReg(RegBase, RegOffset, data); \
} \

#define synopGMACClearBits(RegBase, RegOffset, BitPos)  \
{ \
	u32 data = synopGMACReadReg(RegBase,  RegOffset); \
    data &= (~BitPos); \
	synopGMACWriteReg(RegBase, RegOffset, data); \
