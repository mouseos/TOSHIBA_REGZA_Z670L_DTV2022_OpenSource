...
#ifdef CONFIG_XC5_VIRTUAL_USB
        struct ohci_ed_hw_seg *hw_seg;
  	dma_addr_t		dma_hack;		/* hack addr of ED */
#endif
...
#define ohci_mem_writel(data, addr) do { writel(__le32_to_cpu(data), (volatile void __iomem *)addr);readl((volatile void __iomem *)addr);}while(0);
#define ohci_mem_readl(addr) __raw_readl(addr)
#define ohci_mem_readw(addr) __raw_readw(addr)

struct ohci_td_hw_seg {
	/* first fields are hardware-specified */
...
#define MAXPSW	2
	...
	/* rest are purely for the driver's use */
#ifdef	CONFIG_XC5_VIRTUAL_USB
        struct ohci_td_hw_seg *hw_seg;
	dma_addr_t	td_dma_hack;		/* addr of this TD hack for XC3 */
#endif
	__u8		index;
	struct ed	*ed;
...
//#define ohci_readl(o, addr)     SDUH_readl(SDUH_HCD_REG_BASE, (u32)addr, OHCI_REG)
//#define ohci_writel(o, data, addr)  SDUH_writel(SDUH_HCD_REG_BASE, (u32)addr, (u32)data,  OHCI_REG)
#ifndef CONFIG_XC5_VIRTUAL_USB
inline u32 ohci_readl(const struct ohci_hcd *ohci,__hc32 __iomem * regs)
{
    return SDUH_readl(SDUH_HCD_REG_BASE, (u32)regs, OHCI_REG);
}


inline void ohci_writel (const struct ohci_hcd *ohci, const unsigned int val, __hc32 __iomem *regs)
{
    SDUH_writel(SDUH_HCD_REG_BASE, (u32)regs, (u32)val,  OHCI_REG);
}
#else
...
inline u32 ohci_readl(const struct ohci_hcd *ohci,__hc32 __iomem * regs)
{
    ...
    if (hcd == g_XC_USBH_info.mps_hcd){
       return vusb_readl(VUSB_HCD_REG_BASE, (u32)regs, VUSB_OHCI_REG);
    }
    else{
       return SDUH_readl(SDUH_HCD_REG_BASE, (u32)regs, OHCI_REG);
    }
...
inline void ohci_writel (const struct ohci_hcd *ohci, const unsigned int val, __hc32 __iomem *regs)
{
    struct usb_hcd *hcd = ohci_to_hcd(ohci);
    if (hcd == g_XC_USBH_info.mps_hcd)
        vusb_writel(VUSB_HCD_REG_BASE, (u32)regs, (u32) val, VUSB_OHCI_REG);
   else
       SDUH_writel(SDUH_HCD_REG_BASE, (u32)regs, (u32)val,  OHCI_REG);
}
#endif
	...
	if (big_endian_desc(ohci)) {
		...
		tmp = le32_to_cpup((__force __le32 *)&ohci->hcca->frame_no);
               ...
               if (ohci_to_hcd(ohci) == g_XC_USBH_info.mps_hcd){
                tmp = le32_to_cpu(ohci_mem_readl(&ohci->hcca->frame_no));
	       }else{
		tmp = le32_to_cpup((__force __le32 *)&ohci->hcca->frame_no);
	       }
#endif
                                 ...
                                 const struct td *td, int index)
{
	...
	return (__hc16 *)(big_endian_desc(ohci) ?
	  ...
	  if (ohci_to_hcd(ohci) == g_XC_USBH_info.mps_hcd){
  	  	return (__hc16 *)(big_endian_desc(ohci) ?
			&td->hw_seg->hwPSW[index ^ 1] : &td->hw_seg->hwPSW[index]);
	  }else{
	  	return (__hc16 *)(big_endian_desc(ohci) ?
		&td->hwPSW[index ^ 1] : &td->hwPSW[index]);
	  }
...
#else
	  if (ohci_to_hcd(ohci) == g_XC_USBH_info.mps_hcd){
	  	return hc16_to_cpu(ohci, ohci_mem_readw(ohci_hwPSWp(ohci, td, index)));
	  }else{
	  	return hc16_to_cpup(ohci, ohci_hwPSWp(ohci, td, index));
	  }
