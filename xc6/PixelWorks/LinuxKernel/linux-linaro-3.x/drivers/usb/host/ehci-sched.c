	...
	while (q->ptr) {
		...
		case Q_TYPE_QH:
			if (same_tt(dev, q->qh->dev)) {
				...
				uf = tt_start_uframe(ehci, ehci_mem_readl(&q->qh->hw->hw_info2));
				   }else{
				uf = tt_start_uframe(ehci, q->qh->hw->hw_info2);
				   }
#endif
...
#else
if (ehci == g_XC_USBH_info.mps_ehci){
				type = Q_NEXT_TYPE (ehci, ehci_mem_readl(&hw->hw_next));
}else {
				type = Q_NEXT_TYPE(ehci, hw->hw_next);
}
#endif
...
#else			
			if( ehci == g_XC_USBH_info.mps_ehci){
			type = Q_NEXT_TYPE (ehci, ehci_mem_readl(hw_p));
			}else{
			type = Q_NEXT_TYPE(ehci, *hw_p);
			}
#endif
...
#else
				if( ehci == g_XC_USBH_info.mps_ehci){
				        ehci_mem_writel(ehci_mem_readl(hw_p), &qh->hw->hw_next);
					DBK("*hw_p:%x\n",*hw_p);
				}else {
					qh->hw->hw_next = *hw_p;
				}
...
#else			
			if( ehci == g_XC_USBH_info.mps_ehci){
				ehci_mem_writel(QH_NEXT (ehci,qh->qh_dma), hw_p);	
				DBK("QH_NEXT (ehci,qh->qh_dma):%x\n",QH_NEXT (ehci,qh->qh_dma));
			}
			else {
				*hw_p = QH_NEXT (ehci, qh->qh_dma);
			}
...
#else
		if (ehci == g_XC_USBH_info.mps_ehci){
    ehci_mem_writel(ehci_mem_readl(&hw->hw_info2)&(cpu_to_hc32(ehci, ~(QH_CMASK | QH_SMASK))), &hw->hw_info2);
    ehci_mem_writel(ehci_mem_readl(&hw->hw_info2)|(qh->period? cpu_to_hc32(ehci, 1 << uframe): cpu_to_hc32(ehci, QH_SMASK)), &hw->hw_info2);
    ehci_mem_writel(ehci_mem_readl(&hw->hw_info2)|c_mask, &hw->hw_info2);
		}else{
