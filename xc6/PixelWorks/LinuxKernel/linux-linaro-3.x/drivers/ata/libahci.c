...
#ifdef CONFIG_SATA_VIXS_AHCI
#include "ahci_vixs.h"
int vixs_ahci_scr_read( struct ata_link *link, unsigned int sc_reg, u32 *val)
{
  void __iomem *port_mmio = ahci_port_base(link->ap);
  int offset = ahci_scr_offset(link->ap, sc_reg);

  if (offset) {
    *val = VIXS_SATA_reg_readl(port_mmio + offset, 0);
    return 0;
  }
  return -EINVAL;
}

int vixs_ahci_scr_write(struct ata_link *link, unsigned int sc_reg, u32 val) 
{
  void __iomem *port_mmio = ahci_port_base(link->ap);
  int offset = ahci_scr_offset(link->ap, sc_reg);

  if (offset) {
    VIXS_SATA_reg_writel(val, port_mmio + offset, 0);
    return 0;
  }
  return -EINVAL;
}
...
#ifdef CONFIG_VIRTUAL_XC5_SATA
int vsata_ahci_scr_read( struct ata_link *link, unsigned int sc_reg, u32 *val)
{
  void __iomem *port_mmio = ahci_port_base(link->ap);
  int offset = ahci_scr_offset(link->ap, sc_reg);

  if (offset) {
    *val = VIXS_VSATA_reg_readl(port_mmio + offset, 0);
    return 0;
  }
  return -EINVAL;
}

int vsata_ahci_scr_write(struct ata_link *link, unsigned int sc_reg, u32 val) 
{
  void __iomem *port_mmio = ahci_port_base(link->ap);
  int offset = ahci_scr_offset(link->ap, sc_reg);

  if (offset) {
    VIXS_VSATA_reg_writel(val, port_mmio + offset, 0);
    return 0;
  }
  return -EINVAL;
}
